...
/*   Updated: 2024/06/17 18:46:39 by fwahl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_coords	check_wall(t_map *map, t_coords pos, t_coords pos_add)
{
	while (pos.x >= 0 && pos.x / FIELD_SIZE < map->max.x && pos.y >= 0 && pos.y / FIELD_SIZE < map->max.y)
	{
		if (map->map[pos.y / FIELD_SIZE][pos.x / FIELD_SIZE] == '1')
			break ;
		else
		{
			pos.x += pos_add.x;
...
static t_coords	horizontal_line(t_map *map, t_player *player, double radian)
{
	t_coords	pos;
	t_coords	pos_add;
	...
	if (radian > 0 && radian < PI_ONE) // up
	{
		...
		pos = check_wall(map, pos, pos_add);
	}
...
static t_coords	vertical_line(t_map *map, t_player *player, double radian)
{
	t_coords	pos;
	t_coords	pos_add;
...
static int	wall_distance(t_window *window, t_coords player, t_coords horizontal, t_coords vertical)
{
	double	distance_h;
	double	distance_v;
